...
__device__ char background_noise(const float r, const CTYPE time){
	char flag = 0;
	flag = ( (0.0 <= time ) && PoissonProcess( r, time, 4.0, 0.0 ) );
	return (flag)?1:0;
}
...
__host__ void setInputStim( InputFunctionsStruct *List,  Neuron*host_neurons, int *NeuronTypeID, enum NeuronType type, const char * file_name ){
    FILE *fp;
