	...
	while(true){
		...
		for(int target_row = 0; target_row < T_cpu_sim; target_row++){
			for(int i = h->gpu_connections_num; i < TotalNumOfConnectivityTypes; i++){
                ...
                read_row = (read_row >= 0)? read_row: pv->T_print + target_row - delay ;
...
void* print_spike_train(void *p){
	...
	while(true){
	    pthread_barrier_wait(&ready[id]);
        modulo = ((*count)-1)%(T_print/T_cpu_sim);
        CUDA_SAFE_CALL( cudaMemcpyAsync( &(pv->host_spike[modulo*T_cpu_sim*pv->total_nn]), &(pv->from_spike[modulo*T_cpu_sim*pv->total_nn]), sizeof(char)*T_cpu_sim*pv->total_nn, cudaMemcpyDeviceToHost, pv->stream ));
        cudaStreamSynchronize(pv->stream);
	    pthread_barrier_wait(&A_done[id]);
        if( (*count)%(T_print/T_cpu_sim) == 0){
            n = pv->n  - T_print;
...
///////////////////////////////////
	...
	*cpu_sim_threads = (pthread_t *)malloc(sizeof(pthread_t)*2);
	...
	for(int th = 0; th < 2; th++){
		...
		h->spike =  (char *)malloc(sizeof(char)*h->neuron_num*T_cpu_sim);
