...
///////////////////////////////////
	...
	*Host_sim = Host_sim_tmp;
    ...
    for(int i = 0; i < gpu_connections_num; i++){
        ...
        int end = 0;
        int width = 0;
        fprintf(stderr, "%s\n", host_Connectivities[i].filename );
        for(int dev_id = 0; dev_id < DEV_NUM; dev_id++){
            cudaSetDevice(dev_id);
			...
			if( con->max_conv > 5000  ){
                ...
                if(con->type == parallel_fiber_to_stellate) con->pr = 8;
			}
...
*/          
            ...
            d_sec += (double)rsec + (double)nsec / (1000*1000*1000);
        ...
        fprintf(stderr, "total: %lf trial:%d\n avg/trial = %lf\n", d_sec, TRIALS, d_sec/(double)TRIALS);
