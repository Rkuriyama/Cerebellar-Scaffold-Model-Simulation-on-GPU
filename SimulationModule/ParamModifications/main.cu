		...
		for(int target_row = 0; target_row < pv->delay_max_row; target_row++){
			for(int i = h->gpu_connections_num; i < TotalNumOfConnectivityTypes; i++){
				...
				host_calculate_current_diff( preType, postNeuron->num, postNeuron->base_id,
						dg, h->refractory_time_left , c->host_rptr, c->cindices,  c->val, w_bar,
...
/*
                    cudaDeviceSynchronize();
                    if(dev_id > 0)           CUDA_SAFE_CALL( cudaMemcpyPeer( &Dev[dev_id - 1].spike[ target_row*Dev[dev_id - 1].total_neuron_num + Dev[dev_id - 1].neuron_num + Dev[dev_id -1].pre_neuron_num], dev_id - 1,   &Dev[dev_id].spike[target_row*Dev[dev_id].total_neuron_num], dev_id, sizeof(char)*Dev[dev_id].neuron_num ) );
                    if(dev_id < DEV_NUM - 1) CUDA_SAFE_CALL( cudaMemcpyPeer( &Dev[dev_id + 1].spike[ target_row*Dev[dev_id + 1].total_neuron_num + Dev[dev_id + 1].neuron_num                                ], dev_id + 1,   &Dev[dev_id].spike[target_row*Dev[dev_id].total_neuron_num], dev_id, sizeof(char)*Dev[dev_id].neuron_num ) );
                    cudaDeviceSynchronize();
...
*/          
            ...
            rsec = end_time.tv_sec - start_time.tv_sec;
            nsec = end_time.tv_nsec - start_time.tv_nsec;
            d_sec[trial] = (double)rsec + (double)nsec / (1000*1000*1000);
            d_sec_sum += d_sec[trial];
        ...
        // std
        double d_sec_std = 0;
        double d_sec_mean = d_sec_sum/(double)TRIALS;
        double d_sec_max = 0;
        for(int i = 0; i < TRIALS; i++){
            d_sec_std += (d_sec[i] - d_sec_mean)*(d_sec[i] - d_sec_mean);
            d_sec_max = (d_sec_max < d_sec[i])? d_sec[i] : d_sec_max;
        }
        d_sec_std /= (double)TRIALS;
        d_sec_std = sqrt(d_sec_std);


        fprintf(stderr, "total: %lf trial:%d\n avg/trial = %lf std = %lf max = %lf\n", d_sec_sum, TRIALS, d_sec_mean, d_sec_std, d_sec_max);
        ////
