...
__host__ void host_calculate_current_diff(const int preType, const int postNum, const int post_base_id, CTYPE *dg,const int *refractory_time_left , const unsigned int *rptr, const unsigned int *cindices,  const CTYPE *weight, const CTYPE w_bar, const int target_row, const int target_block,  const Sim_cond_lif_exp *Dev);
