                ...
                while(n < T_MAX){
                    ...
                    for(int i=0;i < d->gpu_connections_num ;i++){ 
                        ...
                        if( c->pr == -1  ){
                            ...
                            spike_propagation_mThreads<<<(postNeuron->num+127)/128, 128, 0, d->streams[postType]>>>( postNeuron->base_id, postNeuron->num, dg, c->max_conv, c->ELL_cindices, c->ELL_val, w_bar, d->spike, read_row*(d->pre_neuron_num+d->neuron_num+d->next_neuron_num) + preNeuron->base_id, c->pr );
                        }else{
