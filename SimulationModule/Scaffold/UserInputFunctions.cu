...
__host__ void setInputStim( InputFunctionsStruct *List,  Neuron*host_neurons, int *NeuronTypeID, enum NeuronType type, const char * file_name, int func_id ){
    FILE *fp;

    if( (fp = fopen( &(file_name[0]), "r")) == NULL){
...
__host__ void InitInputStimulation( InputFunctionsStruct *List, Neuron *host_neurons, int *NeuronTypeID){
    ...
    setInputStim( &(List[stim_id]), host_neurons, NeuronTypeID, glomerulus, "background_noise.dat", stim_id);
    stim_id++;

    setInputStim( &(List[stim_id]), host_neurons, NeuronTypeID, glomerulus, "tone_stim.dat", stim_id);
    stim_id++;
