...
void *SimulationOnCPU(void *p){
    ...
    int TNoCoT = pv->num_of_connectivity_types;
    int TNoCeT = pv->num_of_neuron_types;
	...
	while(true){
		...
		for(int target_row = 0; target_row < T_cpu_sim; target_row++){
			for(int i = h->gpu_connections_num; i < TNoCoT; i++){
...
void Initialization( NetworkEnvironment *env, cpu_sim_thread_val **Host_sim, pthread_t **cpu_sim_threads, int *NeuronTypeID, int *ConnectivityTypeID, Neuron *host_Neurons, Connectivity *host_Connectivities, InputFunctionsStruct *host_InputStimList, int T_print, int T_cpu_sim, int total_nn){
    ...
    int TNoCoT = env->num_of_connectivity_types;
    int TNoCeT = env->num_of_neuron_types;

    Sim_cond_lif_exp *Dev = env->Dev;
    // P2P Setting
	...
	ReverseNeuronTypeID = (int *)malloc( sizeof(int)* TNoCeT);
	for(int i = 0; i < TNoCeT; i++){
    ...
    for (int dev_id = 0; dev_id < DEV_NUM; dev_id++){
        ...
        CUDA_SAFE_CALL(cudaMalloc(&Dev[dev_id].dev_neurons,sizeof(Neuron)*TNoCeT));
        CUDA_SAFE_CALL(cudaMemcpy(Dev[dev_id].dev_neurons, neurons, sizeof(Neuron)*TNoCeT, cudaMemcpyHostToDevice));
...
///////////////////////////////////
    ...
    for(int dev_id = 0; dev_id < DEV_NUM; dev_id++){
        ...
        for(int type = 0,i=0; type < TNoCeT; type++){
	...
	*cpu_sim_threads = (pthread_t *)malloc(sizeof(pthread_t)*2);
	...
	for(int i = gpu_neurons_num; i < TNoCeT; i++){
	...
	for(int th = 0; th < 2; th++){
		...
		memcpy( &h->neurons[0], host_Neurons, sizeof(Neuron)*TNoCeT);
		...
		for(int i = gpu_neurons_num, base_id = 0; i < TNoCeT; i++){
		...
		for(int i = gpu_connections_num; i < TNoCoT; i++){
            int *ell_cindices;
			CTYPE *ell_val;
		...
		Host_sim_tmp[th].num_of_neuron_types = env->num_of_neuron_types;
		Host_sim_tmp[th].num_of_connectivity_types = env->num_of_connectivity_types;
...
void InitializeNetworkEnv( NetworkEnvironment *env, int T_print, int T_cpu_sim, std::string dir ){
    //Init Neurons
    init_neurons_params( env->Host_Neurons, env->NeuronTypeID, dir );
    env->total_nn = set_base_id(env->Host_Neurons); // envに保持する必要ない気がするけど、デバッグとかで使うかもしれないので維持

    //Init Connectivities
    init_connectivity_params( env->Host_Connectivities, env->Host_Neurons, env->NeuronTypeID, env->ConnectivityTypeID, dir );
    
    //Init Input Stim
    InitInputStimulation( env->Host_InputStimList, env->Host_Neurons, env->NeuronTypeID, dir );
    ...
    Initialization( env, &(env->Host_sim_val), &(env->cpu_sim_threads), env->NeuronTypeID, env->ConnectivityTypeID, env->Host_Neurons, env->Host_Connectivities, env->Host_InputStimList, env->T_print, env->T_cpu_sim, env->total_nn );
