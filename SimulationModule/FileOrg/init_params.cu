...
void LoadConnectivityFile_ELL(const std::string file_name, unsigned int max_conv, int **d_cindices, CTYPE **d_val, CTYPE weight ,int PreSN_num,int PostSN_num){
...
int GetMaxConv( const std::string file_name, int PreSN_num, int PostSN_num ){
    //max_convergence = #entities_per_row
...
int LoadConnectivityFile(const std::string file_name,unsigned int **host_rptr, unsigned int **d_rptr, unsigned int **d_cindices, CTYPE **d_val, CTYPE weight ,int PreSN_num,int PostSN_num){
...
int set_neuron_params(Neuron *n,enum NeuronType type,const std::string file_name, char duplicate, int num, CTYPE Cm, CTYPE El, CTYPE dt_ref, CTYPE Ie, CTYPE Vr, CTYPE Vth, CTYPE gL, enum DeviceType device_type, int c_num, ...){
	static int CPU = 0;
	static int GPU = 0;
	int target = 0;
    ...
    fprintf(stderr,"\t%s: %d %d\n", file_name.c_str(), target, type);

	n[target].type = target;
	strcpy(n[target].filename, file_name.c_str());
	n[target].num = num;
	n[target].Cm = Cm;
	n[target].El = El;
...
int set_connectivity_params(Connectivity *c, Neuron *neurons, enum ConnectionType type, const std::string file_name, int preNum, int postNum, int preType, int postType, CTYPE initial_weight, CTYPE delay, int UseParallelReduction  ){
	static int GPU = 0;
	static int CPU = 0;
	int target = 0;
    ...
    c[target].max_conv = GetMaxConv( file_name, preNum, postNum );
	c[target].pr = (UseParallelReduction);

    LoadConnectivityFile_ELL( file_name, c[target].max_conv, &c[target].ELL_cindices, &c[target].ELL_val, initial_weight, preNum, postNum );
...
void init_neurons_params( Neuron *Neurons, int *NeuronTypeID, std::string dir){
...
*/
	...
	NeuronTypeID[dcn_cell] = set_neuron_params(
        ...
        2,
        1.0, 0.0, 1.0, 7.1, //exc
        4.0, -85.0, 1.0, 13.6 //26.6 //inh
	);
}

void init_connectivity_params(Connectivity *connectivities, Neuron *neurons, int *NeuronTypeID, int *ConnectivityTypeID, std::string dir){
	...
	ConnectivityTypeID[parallel_fiber_to_purkinje] = set_connectivity_params(
		...
		dir+"parallel_fiber_to_purkinje.dat",
		granule_cell_num,
		purkinje_cell_num,
	...
	ConnectivityTypeID[parallel_fiber_to_golgi] = set_connectivity_params(
		...
		dir+"parallel_fiber_to_golgi.dat",
		granule_cell_num,
		golgi_cell_num,
		NeuronTypeID[granule_cell],
	...
	ConnectivityTypeID[parallel_fiber_to_stellate] = set_connectivity_params(
		...
		dir+"parallel_fiber_to_stellate.dat",
		granule_cell_num,
		stellate_cell_num,
		NeuronTypeID[granule_cell],
	...
	ConnectivityTypeID[parallel_fiber_to_basket] = set_connectivity_params(
		...
		dir+"parallel_fiber_to_basket.dat",
		granule_cell_num,
		basket_cell_num,
		NeuronTypeID[granule_cell],
	...
	ConnectivityTypeID[aa_to_golgi] = set_connectivity_params(
		connectivities,
		neurons,
		aa_to_golgi,
		dir+"ascending_axon_to_golgi.dat",
		granule_cell_num,
		golgi_cell_num,
		NeuronTypeID[granule_cell],
		NeuronTypeID[golgi_cell],
	...
	ConnectivityTypeID[aa_to_purkinje] = set_connectivity_params(
		connectivities,
		neurons,
		aa_to_purkinje,
		dir+"ascending_axon_to_purkinje.dat",
		granule_cell_num,
		purkinje_cell_num,
		NeuronTypeID[granule_cell],
		NeuronTypeID[purkinje_cell],
