...
int LoadConnectivityFile(const char *file_name,unsigned int **host_rptr, unsigned int **d_rptr, unsigned int **d_cindices, CTYPE **d_val, CTYPE weight ,int PreSN_num,int PostSN_num, char preSN_duplicate, enum DeviceType post_dev_type){
	PreSN_num = (PreSN_num < 1)? 1: PreSN_num;
	//PostSN_num = (PostSN_num < 1)? 1000: PostSN_num;

    int range[2][DEV_NUM+1] = {0};
    range[0][0] = 0;
    range[1][0] = 0;
    for(int i = 1; i < DEV_NUM; i++){
        range[0][i] = range[0][i-1] + (PreSN_num + DEV_NUM - 1)/DEV_NUM;
        range[1][i] = range[1][i-1] + (PostSN_num + DEV_NUM - 1)/DEV_NUM;
    }
    range[0][DEV_NUM] = PreSN_num;
    range[1][DEV_NUM] = PostSN_num;

    for(int i = 0; i < DEV_NUM + 1; i++){
        fprintf(stderr, (i != DEV_NUM)?"%d-":"%d\n", range[0][i]);
    }
	...
	int prev_post_id = 0;
	int post_id;
    int area = 0, prev_area=0;
    int removed_synapse = 0;
	rptr[0] = 0;
	while( fgets(str, 256, fp) != NULL ){
		...
		if(post_id > prev_post_id) {
            ...
            if( post_id >= range[1][area+1] ){
                prev_area = area;
                area = area + 1; // 本当は2個先に飛ぶ可能性もあるが今回はそこのvalidationなし。
            }
		}else if(post_id < prev_post_id){
            fprintf(stderr, "connectivity load error: %s is unsorted.\n", file_name);
            exit(1);
        }

        if( cindices[i] >= PreSN_num || post_id >= PostSN_num ){
            fprintf(stderr, "connectivity file-%s corupsed. loaded id exceed the limit number.\n", file_name );
            fprintf(stderr, "load(%d,%d), limit(%d,%d)\n", cindices[i], post_id, PreSN_num, PostSN_num );
            exit(1);
        }else if( !(preSN_duplicate || post_dev_type == OUTPUT ) && ( (area > 0 && cindices[i] < range[0][area - 1]) || (area + 1  < DEV_NUM && cindices[i] >= range[0][area + 2] ) ) ){
            //fprintf(stderr, "DEV_NUM %d is not allowed for %s.\n", DEV_NUM, file_name);
            //fprintf(stderr, "range: [%d-%d]\tid:%d\n", range[0][area - 1], range[0][area+2], cindices[i]);
            //if( cindices[i] < range[0][area - 1] ) fprintf(stderr, "%d < %d - area:%d post:%d\n", cindices[i], range[0][area - 1], area, post_id);
            if( area + 1 < DEV_NUM && cindices[i] >= range[0][area + 2] ) fprintf(stderr, "%d >= %d - area:%d post:%d\n", cindices[i], range[0][area + 2], area, post_id);
            removed_synapse++;
    ...
    if(removed_synapse) fprintf(stderr, "%d synapses were removed (out of range).\n", removed_synapse);
...
int set_connectivity_params(Connectivity *c, Neuron *neurons, enum ConnectionType type,const char* filename, int preNum, int postNum, int preType, int postType, CTYPE initial_weight, CTYPE delay, int UseParallelReduction  ){
	...
	strcpy(c[target].filename, filename);
	c[target].type = type;
	...
	c[target].max_conv = LoadConnectivityFile(filename,&c[target].host_rptr, &c[target].rptr, &c[target].cindices, &c[target].val,initial_weight, preNum, postNum, neurons[preType].duplicate, neurons[postType].dev_type );
...
void init_connectivity_params(Connectivity *connectivities, Neuron *neurons, int *NeuronTypeID, int *ConnectivityTypeID){
	...
	ConnectivityTypeID[dcn_interneuron_to_io] = set_connectivity_params(
		connectivities,
		neurons,
