...
__device__ char background_noise(const float r, const CTYPE time){
	char flag = 0;
	flag = ((0.0 <= time && time < 1000.0) && PoissonProcess( r, time, 4.0, 0.0 ) );
	return (flag)?1:0;
}

__device__ char visual_stim(const float r, const CTYPE time){
	char flag = 0;
    CTYPE fleq;
    fleq = SinusoidalOscillation( 30.-4., 15., 2. ,0, time  )+4;
	flag = ((0.0 <= time ) && PoissonProcess( r, time, fleq, 0.0 ) );
	return (flag)?1:0;
}

__device__ char retinal_stim(const float r, const CTYPE time){
	char flag = 0;
    CTYPE fleq;
...
__device__ pointFunction_t d_pInputFunctions[] = {background_noise, visual_stim, retinal_stim};
